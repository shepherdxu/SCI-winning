...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
                              search_in=(zig_rir3d.__path__[0], "training", "network_training"),
                              base_module='zig_rir3d.training.network_training'):
    ...
    if task == 'Task001_ACDC':
        ...
        plans['plans_per_stage'][Stage]['patch_size'] = np.array([16, 192, 192])
        ...
        plans['plans_per_stage'][Stage]['batch_size'] = 4
        plans['plans_per_stage'][0]['batch_size'] = 4  # stage0和1的batch_size均改为4
        plans['plans_per_stage'][Stage]['patch_size'] = np.array([64, 192, 192])
